...
\begin{doublespacing}\hypersetup{
  ...
  \centerline{Versione aggiornata al 28/04/2021}
\end{doublespacing}
\newpage
...
\begin{itemize}
  \item Formula fondamentale \( \dfrac{dk}{dt} = \dfrac{\mathfrak{F} }{\hbar} \), \( \mathfrak{F}  \) forza applicata
        \begin{itemize}
          \item Modello di Drude \( \dfrac{dk}{dt} -\dfrac{k}{\tau_m}= \dfrac{\mathfrak{F} }{\hbar} \)
          \item Soluzione \( k = \dfrac{q \tau_m F}{\hbar} \left( 1 - e^{\sfrac{t}{\tau_m}} \right) \)
        \end{itemize}
  \item Oscillazioni di Bloch \( \omega = \dfrac{a q F}{\hbar} \), \( \nu = \dfrac{a q F}{h} \) con \( a \) proveniente dalla relazione di dispersione
